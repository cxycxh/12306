...
STATION_DATES = ticket_config['STATION_DATES']

# 填入需要购买的车次(list)，"G1353"
# 修改车次填入规则，注：(以前设置的车次逻辑不变)，如果车次填入为空，那么就是当日乘车所有车次都纳入筛选返回
# 不填车次是整个list为空才算，如果不是为空，依然会判断车次的，这种是错误的写法 [""], 正确的写法 []
STATION_TRAINS = ticket_config['STATION_TRAINS']

# 出发城市，比如深圳北，就填深圳就搜得到
FROM_STATION = ticket_config['FROM_STATION']

# 到达城市 比如深圳北，就填深圳就搜得到
TO_STATION = ticket_config['TO_STATION']
...
# "无座",
# "动卧",
SET_TYPE = ticket_config['SET_TYPE']

# 当余票小于乘车人，如果选择优先提交，则删减联系人和余票数一致在提交
# bool
IS_MORE_TICKET = ticket_config['IS_MORE_TICKET']

# 乘车人(list) 多个乘车人ex:
# "张三",
# "李四"
TICKET_PEOPLES = ticket_config['TICKET_PEOPLES']

# 12306登录账号
USER = ticket_config['USER']
PWD = ticket_config['PWD']

# 加入小黑屋时间默认为5分钟，此功能为了防止僵尸票导致一直下单不成功错过正常的票
TICKET_BLACK_LIST_TIME = ticket_config['TICKET_BLACK_LIST_TIME']

# 自动打码
IS_AUTO_CODE = ticket_config['IS_AUTO_CODE']

# 设置2本地自动打码，需要配置tensorflow和keras库，3为云打码，由于云打码服务器资源有限(为2h4C的cpu服务器)，请不要恶意请求，不然只能关闭服务器
# ps: 请不要一直依赖云服务器资源，在此向所有提供服务器同学表示感谢
AUTO_CODE_TYPE = ticket_config['AUTO_CODE_TYPE']

# 此处设置云打码服务器地址，如果有自建的服务器，可以自行更改
#HOST = "120.24.62.193"
HOST = ticket_config['HOST']
#HOST = "captcha:80"
REQ_URL = ticket_config['REQ_URL']
HTTP_TYPE = ticket_config['HTTP_TYPE']
...
#  password: "授权码"
#  host: "smtp.qq.com"
EMAIL_CONF = ticket_config['EMAIL_CONF']

# 是否开启 server酱 微信提醒， 使用前需要前往 http://sc.ftqq.com/3.version 扫码绑定获取 SECRET 并关注获得抢票结果通知的公众号
SERVER_CHAN_CONF = ticket_config['SERVER_CHAN_CONF']

# 是否开启cdn查询，可以更快的检测票票 1为开启，2为关闭
IS_CDN = ticket_config['IS_CDN']

# 下单接口分为两种，1 模拟网页自动捡漏下单（不稳定），2 模拟车次后面的购票按钮下单（稳如老狗）
ORDER_TYPE = ticket_config['ORDER_TYPE']

# 下单模式 1 为预售，整点刷新，刷新间隔0.1-0.5S, 然后会校验时间，比如12点的预售，那脚本就会在12.00整检票，刷新订单
#         2 是捡漏，捡漏的刷新间隔时间为0.5-3秒，时间间隔长，不容易封ip
ORDER_MODEL = ticket_config['ORDER_MODEL']
...
# 3、开启代理ip
IS_PROXY = ticket_config['IS_PROXY']

# 预售放票时间, 如果是捡漏模式，可以忽略此操作
OPEN_TIME = ticket_config['OPEN_TIME']
# 1=使用selenium获取devicesID
# 2=使用网页端/otn/HttpZF/logdevice获取devicesId，这个接口的算法目前可能有点问题，如果登录一直302的请改为配置1
# 3=自己打开浏览器在headers-Cookies中抓取RAIL_DEVICEID和RAIL_EXPIRATION，这个就不用配置selenium
COOKIE_TYPE = ticket_config['COOKIE_TYPE']
# 如果COOKIE_TYPE=1，则需配置chromeDriver路径,下载地址http://chromedriver.storage.googleapis.com/index.html
# chromedriver配置版本只要和chrome的大版本匹配就行
CHROME_PATH = ticket_config['CHROME_PATH']

# 为了docker37 准备的环境变量，windows环境可以不用管这个参数
CHROME_CHROME_PATH = ticket_config['CHROME_CHROME_PATH']

# 如果COOKIE_TYPE=3, 则需配置RAIL_EXPIRATION、RAIL_DEVICEID的值
RAIL_EXPIRATION = ticket_config['RAIL_EXPIRATION']
RAIL_DEVICEID = ticket_config['RAIL_DEVICEID']
...
RANDOM_AGENT = ticket_config['RANDOM_AGENT']

PASSENGER_TICKER_STR = ticket_config['PASSENGER_TICKER_STR']

# 保护12306官网请求频率，设置随机请求时间，原则为5分钟不大于80次
# 最大间隔请求时间
MAX_TIME = ticket_config['MAX_TIME']
# 最小间隔请求时间
MIN_TIME = ticket_config['MIN_TIME']

# 软件版本
RE_VERSION = ticket_config['RE_VERSION']
